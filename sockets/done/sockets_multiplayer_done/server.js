...
let players = [];
let words = ['ko', 'ged', 'tissemyre', 'myresluger'];
...
function nySocket(socket) {
    ...
    socket.on('brugernavn', addPlayers);
    function addPlayers(brugernavn){
      console.log('Modtager brugernavn ' + brugernavn + ' fra ' + socket.id);
      ...
      if(players.length == noOfPlayers)beginRound();
      function beginRound(){
        console.log("starter runde");
        startTimer();
        players[whosTurn].turn = true;
        players[whosTurn].word = words[0];
        io.sockets.emit('beginRound', players);
        words.splice(0,1);
        console.dir(words);
      }
    }
